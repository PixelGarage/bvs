          ...
          if (column.hasClass('menu-views') && _isNormalLayout) {
            ...
            if (_ulsColumn2.length > 0) {
              ...
              if (_ulsColumn3.length > 0) {
                // move content of second column (Information) to third column (Ausbildungskonzept|empty)
                // fill emptied column with third views column
                if(column.siblings().length == 1) {
                  // create a dummy second sibling
                  column.parent().append('<li></li>');
                }
                var _sibls = column.siblings();
                var _sibl1 = _sibls.first();

                _sibl1.children().detach().prependTo(_sibls.last());
                _ulsColumn3.detach().appendTo(_sibl1).wrapAll('<div class="view-content-third clearfix"></div>');
              }
            }
          }
        }
...
*/
      ...
      $consultingForm.bind('submit', function (event) {
        ...
        // get form values
        var label = $(this).find('#webform-component-interesse select option:selected').html();
